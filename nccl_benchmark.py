#!/usr/bin/env python3
"""
NCCL AllReduce Benchmark Script
Runs AllReduce operations using NCCL backend for benchmarking
"""
...
def benchmark_allreduce(rank, world_size, master_addr, master_port, elements, iteration_time, device_name):
    """Run AllReduce benchmark using NCCL"""
    
    # Set environment variables for NCCL
    os.environ['MASTER_ADDR'] = master_addr
    os.environ['MASTER_PORT'] = str(master_port)
    os.environ['NCCL_IB_DISABLE'] = '1'  # Disable InfiniBand, use TCP
    os.environ['NCCL_IGNORE_DISABLED_P2P'] = '1'  # Ignore disabled P2P between GPUs
    if device_name:
        os.environ['NCCL_SOCKET_IFNAME'] = device_name
    ...
    if torch.cuda.is_available():
        ...
        # Since ranks are incremented by 2 in the script (0, 2, 4, 6, 8...),
        # we divide by 2 to get sequential GPU assignment: rank 0->GPU0, rank 2->GPU1, rank 4->GPU2, etc.
        # This ensures no duplicate GPU assignment when ranks wrap around
        gpu_id = (rank // 2) % num_gpus
        device = torch.device(f'cuda:{gpu_id}')
        torch.cuda.set_device(device)
        print(f"Rank {rank}: Using GPU {gpu_id} (total GPUs: {num_gpus}, device: {device})")
    else:
        device = torch.device('cpu')
        print(f"Rank {rank}: CUDA not available, using CPU")
    
    # Create tensor for AllReduce
...
def main():
    if len(sys.argv) < 8:
        print("Usage: python nccl_benchmark.py <rank> <world_size> <master_addr> <master_port> "
